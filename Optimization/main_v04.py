...
'''
steel_plants_data = pd.read_csv('optimization/input/Standorte.csv',encoding='iso-8859-1')

optimization_horizon = 24*7    #in hours 

EAF_plants = steel_plants_data.loc[steel_plants_data['Process Chain'] == 'Scrap-EAF']
...
steel_plants['total production'] = steel_plants['hourly production']*optimization_horizon

#%%

steel_plant_flex = pd.DataFrame()
steel_plants_pnom = []
...
#for i in range(0,3):
    ...
    steel_plants_pnom.append(limits['Total_max'])
    ...
    base_model = Price_Opt(price_data=price_data,
                       ...
                       steel_prod=steel_plants['total production'].iloc[i],
    ...
    steel_plant_flex[str(steel_plants['bus'][i])+ '_pos' ] = pos_flex_total/limits['Total_max']
    steel_plant_flex[str(steel_plants['bus'][i])+ '_neg'] = -1*np.array(neg_flex_total)/np.array(limits['Total_max'])


# %%
steel_plants['pnom'] = steel_plants_pnom

steel_plant_flex.to_csv('flex_amt_v2.csv')
steel_plants.to_csv('Steel_Plant_Data/updated_from_opt_v2.csv')
