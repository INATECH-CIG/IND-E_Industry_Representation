...
def get_values(model,optimization_horizon, price_data, fuel_data, spec_elec_cons):
...
# list to check flexibility calc in model 
    ...
    for i in range(1,optimization_horizon+1):
        time.append(i)
        elec_price.append(price_data['0'][i])
 ...
 # quick model check that consumption increases with decreasing fuel price 
        ...
        total_fuel_price.append(price_data['0'].iat[i] +\
...
def power_limits(plant_cap, spec_elec_cons, iron_mass_ratio):
    
    liquid_steel_max = plant_cap/0.8
    liquid_steel_min = .25*liquid_steel_max
    
    dri_max = liquid_steel_max*iron_mass_ratio['DRI']
    dri_min = liquid_steel_min*.25
    
    iron_ore_max = dri_max*iron_mass_ratio['iron']
    iron_ore_min =  dri_min* iron_mass_ratio['iron']
